...
前往 [GitHub Releases](https://github.com/44-99/pixel-agent-buddy/releases) 下载 Windows 安装包或便携版。下载版自带 Hook 执行环境，不再要求单独安装 Node.js；首次启动只会询问连接本机检测到的受支持 Agent。
...
按住鼠标左键可拖动宠物。右键菜单只保留 Hooks 状态与修复、开机启动、隐藏和退出；动画状态完全由真实 Agent 事件驱动，不再提供手动演示按钮。
